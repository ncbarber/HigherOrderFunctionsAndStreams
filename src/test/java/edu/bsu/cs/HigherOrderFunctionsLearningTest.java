...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(str -> str.substring(0, 1)).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(str -> Integer.parseInt(str.substring(1))%2 == 0).collect(Collectors.joining());
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(x -> x.timestamp.isBefore(instant)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = Collections.max(input.collect(Collectors.groupingBy(x -> x.timestamp.atZone(ZoneId.of("UTC")).getMonth(),Collectors.counting())).entrySet(), Map.Entry.comparingByValue()).getKey();
